...
function initNs() {
   ...
   let verticalConstant = (crStng.isVertical) ? 1 : 0 ;
    ...
    if (crStng.mode == 0 || crStng.mode == 1) {
        if(!crStng.iconsToRight){
        nsLayout.attach(tsIcon, 0, 1, 1, 1);
        nsLayout.attach(tsLabel, 1, 1, 1, 1);
        }else{
        nsLayout.attach(tsIcon, 1, 1, 1, 1);
        nsLayout.attach(tsLabel, 0, 1, 1, 1);
        }
        ...
        if (crStng.showTotalDwnld) {
            ...
            nsLayout.attach(tdLabel, 3 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
                ...
                nsLayout.attach(tdIcon,  3 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
                nsLayout.attach(tdLabel, 2 - 2*verticalConstant, 1 + verticalConstant, 1, 1);                
            }
        }
    }
        ...
        if (crStng.revIndicator) {
            if(!crStng.iconsToRight){
                ...
                nsLayout.attach(usIcon,  1, 1, 1, 1);
                nsLayout.attach(usLabel, 0, 1, 1, 1);
                nsLayout.attach(dsIcon,  3 - 2*verticalConstant, 1 + verticalConstant, 1, 1); 
                nsLayout.attach(dsLabel, 2 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
            }
        }
        else {
            if(!crStng.iconsToRight){
            ...
            nsLayout.attach(usLabel, 3 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
                ...
                nsLayout.attach(dsLabel, 0, 1, 1, 1);
                nsLayout.attach(dsIcon,  1, 1, 1, 1); 
                nsLayout.attach(usLabel, 2 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
                nsLayout.attach(usIcon,  3 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
            }
        }
        ...
        if (crStng.showTotalDwnld) {
            ...
            nsLayout.attach(tdLabel, 6 - 2*verticalConstant, 1 + verticalConstant, 1, 1);
                ...
                nsLayout.attach(tdIcon,  6 - 2*verticalConstant, 1 + verticalConstant, 1, 1); 
            }
        }
    }
    else {
        ...
        nsLayout.attach(tdLabel, 1, 1, 1, 1);
            ...
            nsLayout.attach(tdLabel, 0, 1, 1, 1);
            nsLayout.attach(tdIcon,  1, 1, 1, 1);
        }
    }
